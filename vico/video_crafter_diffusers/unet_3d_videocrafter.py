...
class UNet3DVideoCrafterConditionModel(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    r"""
    ...
    def forward(
        ...
        if self.fps_cond:
            ...
            fps_emb = fps_emb.to(sample.device, dtype=self.dtype)
            emb += self.fps_embedding(fps_emb).repeat_interleave(repeats=num_frames, dim=0)
